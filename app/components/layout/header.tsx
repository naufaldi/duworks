import { Box, Button, Container, Flex, HStack, Link as ChakraLink, useDisclosure, Slide } from '@chakra-ui/react'
      ...
      <Container maxW="1230px" mx="auto" w="full">
        <Flex w="full" justifyContent="space-between" alignItems="center" >
          ...
          <HStack spacing="56px" display={{ base: 'none', sm: 'flex' }}>
            <ChakraLink as={RemixLink} to="/" p="8px">About</ChakraLink>
            <ChakraLink as={RemixLink} to="/" p="8px">Services</ChakraLink>
            <ChakraLink as={RemixLink} to="/" p="8px">Portfolio</ChakraLink>
          </HStack>
          <Button
          ...
          <Button display={{ base: 'flex', sm: 'none' }} width={"32px"} height={'32px'} zIndex={'15'} border="0" onClick={onToggle}>
            {
          ...
          <Flex flexDir={'column'} gap="56px" alignItems={'center'}>
            {
              listMenuMobile.map((item, index) => (
                <ChakraLink as={RemixLink} key={index} textAlign={'center'} onClick={onToggle} to={item.link} py="12px" maxW="210px" w="full" >
                  {item.caption}
                </ChakraLink>
              ))
            }
